...
namespace Microsoft.BizTalk.FileZ.Adapter
{
    ...
    /// </summary>
    internal class FileZReceiverEndpoint : ReceiverEndpoint
    {
        public FileZReceiverEndpoint()
        {
        }
        ...
        {
            Trace.WriteLine("[FileZReceiverEndpoint] Open called");
            this.errorCount = 0;

            this.properties = new FileZReceiveProperties();
        ...
        {
            Trace.WriteLine("[FileZReceiverEndpoint] Updated called");
            lock (this)
            {
                Stop();
        ...
        {
            Trace.WriteLine("[FileZReceiverEndpoint] Dispose called");
            //  stop the schedule
            Stop();
        }
        ...
        {
            Trace.WriteLine("[FileZReceiverEndpoint] PickupFilesAndSubmit called");
            ...
            Trace.WriteLine(string.Format("[FileZReceiverEndpoint] Found {0} files.", items.Length));
        ...
        private void SubmitFiles(List<BatchMessage> files)
        {
            ...
            Trace.WriteLine(string.Format("[FileZReceiverEndpoint] SubmitFiles called. Submitting a batch of {0} files.", files.Count));
        ...
        private class BatchInfo
        {
            ...
            {
                Trace.WriteLine(string.Format("[FileZReceiverEndpoint] OnBatchComplete called. overallStatus == {0}.", overallStatus));
        ...
        private IBaseMessage CreateMessage (string srcFilePath, string renamedFileName)
        {
            ...
            {
                try { 
                    Trace.WriteLine("[FileZReceiverEndpoint] Renaming file " + srcFilePath);
        ...
        //  properties
        private FileZReceiveProperties properties;
